...
- ✅ 优化 BaudRate 动态切换 - 无需关闭串口，直接修改波特率属性
- ✅ 改进 BaudRate 对话框 - 可编辑下拉框，支持预设和手动输入
- ✅ 改进 SetEndlog 对话框 - 显示正确的转义字符 (`\r\n`、`\r`、`\n`)
- ✅ 新增 modeend 指令 - 结束当前模块定义，支持连续发送时正确分隔模块
- ✅ 修复连续发送特殊指令 - BaudRate/SetEndlog 不再被跳过
...
- **特殊指令系统** - 7 种高级指令（delay、SendHex、BaudRate、SetEndlog、SendMode、StopContinuous、mode），解锁自动化测试能力
