...
def parse_main_version(main_text: str) -> str:
    match = MAIN_PATTERN.search(main_text)
    if not match:
        raise ValueError("main.py 中 TOOL_VERSION 提取失败")
    return match.group(1)
...
def main() -> int:
    if len(sys.argv) != 4:
        return fail(
            "用法: python .github/scripts/validate_serial_tool_release.py <tag> <readme_path> <main_py_path>"
        )

    tag = sys.argv[1]
    readme_path = Path(sys.argv[2])
    main_path = Path(sys.argv[3])

    if not readme_path.exists():
        return fail(f"README 文件不存在: {readme_path}")
    if not main_path.exists():
        return fail(f"main.py 文件不存在: {main_path}")

    try:
        tag_version = parse_tag_version(tag)
        readme_version = parse_readme_version(readme_path.read_text(encoding="utf-8"))
        tool_version = parse_main_version(main_path.read_text(encoding="utf-8"))
