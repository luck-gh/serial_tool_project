...
- ✅ 新增发送响应表达式校验，支持捕获值引用、流式分包匹配、独立超时和失败策略
- ✅ 增加校验结果统计与状态颜色，循环发送可选择最近结果或失败保持红色
- ✅ 优化接收区与发送区搜索，全部结果高亮并区分当前匹配项
- ✅ 发送区搜索支持多选模块，以及按字符串、备注或两者一起搜索
- ✅ 接收区完整保留发送、接收、系统和错误信息，显示来源可随时重新筛选
- ✅ 保存数据时可独立选择输出来源，并持久化显示来源和保存来源配置
- ✅ 完善替换规则弹窗、默认规则、响应设置及操作手册
- ✅ GitHub Release 使用提交信息作为版本说明，并为 EXE 自动追加版本号
