...
```txt
新增运行中配置自动保存和原子写入, 优化串口接收线程空转与停止流程, 完善 Windows EXE 操作手册

主要更新内容:
1. 新增变更自动保存 - 配置, 命令和界面状态变化后执行 800ms 防抖保存
2. 增加定时兜底 - 长时间连续编辑时每 30 秒检查并保存 dirty 状态
3. 保留退出保存 - 正常关闭窗口时继续同步保存最新状态
4. 改用原子写入 - JSON 通过同目录临时文件, fsync 和 os.replace 安全替换
5. 完善状态覆盖 - 命令启用状态, 替换规则, 串口参数, 远程设置和界面布局均触发保存
6. 降低线程空转 - 串口接收改为阻塞等待首字节后批量读取缓冲区数据
7. 优化停止流程 - 使用停止事件和 cancel_read, 由工作线程统一关闭串口句柄
8. 减少退出误报 - 正常停止产生的读取异常不再作为串口错误显示
9. 增加回归检查 - 覆盖原子替换失败保护和实际 Qt 防抖保存
10. 更新用户文档 - 完善 GUI, CLI 的 Windows EXE 操作步骤, 参数和常见问题

修改文件:
- checks/state_persistence_check.py
- core/serial_thread.py
- docs/OPERATION_MANUAL_FOR_CLI.md
...
- **新增** 运行中自动保存
  - 串口参数, 远程设置, 输出筛选, 发送设置和界面折叠状态发生变化后标记 dirty
  - 命令内容, 备注, 启用状态和替换规则变化后同步纳入自动保存
  - 用户停止操作 800ms 后保存, 避免每次按键都写入磁盘
  - 独立 30 秒定时器仅在 dirty 时兜底保存, 避免持续输入导致防抖一直延后
  - 正常关闭窗口时保留最后一次同步保存
- **保护** 初始化加载流程
  - 恢复已有配置期间禁止自动保存
  - 避免控件初始化信号把尚未加载完整的状态覆盖回配置文件
- **改进** JSON 持久化安全
  - 先在配置文件同目录写入唯一临时文件
  - 写完后刷新 Python 缓冲区并执行 `fsync`
  - 使用 `os.replace` 原子替换正式配置
  - 替换失败时保留旧配置并清理临时文件

### 串口接收与停止流程

- **优化** 无数据时的接收行为
  - 由持续查询 `in_waiting` 改为最多阻塞 100ms 等待首字节
  - 首字节到达后一次性读取当前缓冲区剩余数据
  - 减少空闲串口的线程循环次数, 信号发送次数和 UI 刷新压力
- **优化** 串口线程退出
  - 使用线程安全的停止事件协调打开, 读取和退出过程
  - `stop()` 优先调用 pySerial `cancel_read()` 解除阻塞读取
  - 串口句柄只在工作线程的 `finally` 中统一关闭, 避免 Windows 句柄并发释放
  - 用户主动停止时抑制预期内的读取错误和串口占用误报

### 文档与验证

- **增加** `checks/state_persistence_check.py`
  - 验证保存后的 JSON 可正常解析且不残留临时文件
  - 模拟原子替换失败, 确认旧配置内容保持不变
  - 使用无界面 Qt 实例验证控件变化经过防抖后实际写入配置
- **更新** GUI 和 CLI 操作手册
  - 面向 Windows EXE 用户重写启动入口和操作步骤
  - 明确 CLI 参数单位, 配置覆盖规则, 特殊命令范围和退出码
  - 补充 GUI 远程控制, 数据筛选保存和常见故障排查
...
```txt
新增发送响应表达式校验, 完善搜索高亮与范围选择, 重构接收数据来源记录和筛选保存, 优化 GitHub Release 发布信息与文件命名

主要更新内容:
1. 新增响应表达式校验 - 支持引用查找捕获值, 流式分包匹配和单包多响应处理
2. 增加校验控制 - 支持独立超时, 失败继续或停止, 循环颜色策略及错误信息开关
3. 完善校验反馈 - 增加通过统计, 单元格状态颜色和值不一致诊断
4. 优化搜索显示 - 全部匹配项同时高亮, 当前匹配项使用差异化标识
5. 扩展发送区搜索范围 - 支持多选模块及字符串, 备注或一起搜索
6. 重构接收输出记录 - 完整保留发送, 接收, 系统和错误四类信息
7. 新增保存来源筛选 - 保存时可独立选择数据来源并持久化配置
8. 完善替换规则弹窗 - 增加响应匹配开关, 设置弹窗, 默认规则和清除交互
9. 更新操作手册 - 补充响应校验, 搜索范围和数据保存说明
10. 优化自动发布 - Release 使用提交信息作为说明, EXE 文件名自动追加版本号
...
- core/text_search.py
- dialogs/replacement_rule_dialog.py
- dialogs/save_data_dialog.py
- main_window.py
- managers/output_manager.py
- widgets/command_widgets.py
- widgets/find_widgets.py
- docs/OPERATION_MANUAL_FOR_GUI.md
- docs/VERSION_HISTORY.md
- README.md
...
- **新增** 响应表达式校验
  - 响应表达式可通过 `$1`, `${1}` 和 `${name}` 引用查找表达式捕获值
  - 引用值会进行正则安全转义后再参与响应匹配
  - 普通响应正则只负责查找, 捕获值引用同时承担原始数据一致性校验
- **支持** 连续接收流和多任务匹配
  - 响应拆分到多个串口数据包时会保留未完成内容继续等待
  - 单个数据包包含多条响应时可完成多个非重叠任务
  - 同一匹配区间只完成一个任务, 冲突时优先最早发送的任务
  - 较晚任务可在匹配成功时提前完成, 不强制整体 FIFO
- **增加** 独立规则设置
  - 每条规则可配置独立超时时间
  - 失败后可继续发送或停止连续发送
  - 循环发送支持“最近一次结果”和“失败后保持红色”策略
  - 可选择是否在接收区显示匹配系统错误
- **完善** 校验反馈
  - 通过显示淡绿色, 失败或超时显示淡红色
  - 蓝色正常选择优先于状态色, 搜索高亮继续显示在文字片段上
  - 增加校验通过数和已完成数统计
  - 值不一致时输出替换后的发送字符串、期望值和实际值
  - 复位时清除统计、待校验任务和全部状态颜色
...
- **扩展** 发送区搜索范围
  ...
  - 搜索区域支持字符串, 备注或一起搜索
- **修复** 发送区搜索交互
  - 循环发送和界面刷新不再自动跳转到搜索结果
  - 仅在回车或点击上下箭头时切换当前匹配项
  - 避免高亮造成文字重影、位置偏移和首次点击跳变

### 接收记录与保存

- **重构** 接收区输出记录
  - 发送, 接收, 系统和错误信息统一进入完整日志
  - 显示来源只控制当前界面内容, 不再丢弃隐藏信息
  - 修改显示来源后可重新构建接收区并显示此前隐藏的数据
- **新增** 保存数据来源选择
  - 保存时可再次选择发送, 接收, 系统和错误来源
  - 保存筛选基于完整日志, 不受接收区当前显示来源限制
  - 显示来源和保存来源分别写入配置文件并在下次启动恢复

### 规则弹窗与自动发布

- **完善** 全局和单行替换规则弹窗
  - 增加响应表达式输入框、响应匹配开关和设置按钮
  - 未开启响应匹配时相关控件禁用
  - “默认规则”只填充内容并复制当前连续发送间隔作为独立超时
  - “清除规则”只清空内容, 不再直接关闭弹窗
- **优化** GitHub Release
  - 发布说明直接使用标签对应提交的完整提交信息
  - 发布 EXE 自动使用 `vX.Y.Z` 版本后缀
  - 本地 PyInstaller 构建文件名保持不变
...
```txt
完善 GUI 查找与替换发送能力, 重构内置帮助与操作手册, 精简 README 并同步打包文档资源

主要更新内容:
1. 新增接收区与发送编辑区独立查找 - 支持大小写匹配, 正则表达式和全字符匹配
2. 新增替换发送模式 - 支持全局规则和命令行独立规则, 发送时临时替换且不修改原始命令
3. 增强正则替换 - 支持 VSCode 风格 `$1`, `${1}`, `$&`, `$$` 替换写法
4. 优化发送编辑区查找体验 - 匹配项使用黄底黑字高亮, 回车可连续跳转下一项
5. 新增多行规则操作 - 多行选中时可批量设置或清除选中行替换规则
6. 优化发送设置 UI - 结尾标识符和发送选项统一网格布局, 调整连续发送与循环发送顺序
7. 新增快捷键 - F1 打开帮助, Ctrl+O 打开/关闭串口, Ctrl+R 开始/停止连续发送, Ctrl+T 切换替换发送
8. 重构帮助文档 - 新增 GUI/CLI 操作手册, Help 窗口只展示两份操作手册
9. 精简 README - 删除与操作手册重复的细节内容, 保留项目介绍, 快速入口和文档跳转
10. 更新打包资源 - EXE 仅打包 GUI/CLI 两份操作手册, 不打包 README
...
- **新增** 替换发送模式
  - 支持全局替换规则
  - 支持每条命令独立替换规则
  - 替换只发生在发送瞬间, 不修改编辑框原始内容
- **增强** 正则分组替换
  - 支持 `$1`, `${1}` 引用捕获组
  - 支持 `$&` 引用完整匹配
  - 支持 `$$` 输入字面量 `$`
- **优化** 多行规则编辑
  - 多行选中时右键菜单显示为“设置选中行规则”
  - 多行选中时支持批量清除选中行独立规则
  - 拥有独立规则的命令行通过 UI 标记提示

### 发送设置与快捷键

- **优化** 发送设置布局
  - 结尾标识符改为左右网格布局
  - 发送设置区域控件左侧统一对齐, 右侧随侧栏宽度伸缩
  - 调整循环发送与连续发送间隔的上下顺序
- **统一** 替换发送视觉提示
  - 可替换的单条发送按钮使用紫色背景白色文字
  - 全局规则按钮使用紫色背景白色文字
  - 替换模式下连续发送按钮使用紫色背景白色文字
- **新增** 快捷键
  - `F1`: 打开帮助
  - `Ctrl+O`: 打开/关闭串口
  - `Ctrl+R`: 开始/停止连续发送
...
- **新增** `docs/OPERATION_MANUAL_FOR_GUI.md`
  - 集中说明 GUI 操作, 快捷键, 查找, 替换发送, 连续发送和特殊命令
- **新增** `docs/OPERATION_MANUAL_FOR_CLI.md`
  - 集中说明 CLI 启动入口, 参数, 配置文件, 特殊命令支持范围和自动化示例
- **重构** Help 窗口
  - 使用页签展示 GUI/CLI 两份操作手册
  - 不再显示 README
  - 每次打开帮助时重新读取文档内容
  - 修正帮助窗口标题栏和关闭按钮显示
- **精简** README
  - 删除与操作手册重复的操作细节
  - 保留项目概览, 快速启动, 功能概览和文档入口
  - 将具体操作统一跳转到 GUI/CLI 操作手册

### 打包与稳定性

- **调整** PyInstaller 文档资源
  - GUI, 通用版和 CLI 版仅打包两份操作手册
  - README 不再打包进 EXE
- **修复** 启动时短暂弹出小窗口的问题
  - 调整查找栏相关控件的父级归属
  - 减少初始化阶段的瞬时独立窗口显示
- **优化** 命令表格按钮样式刷新
  - 使用属性和统一样式减少频繁 setStyleSheet 的风险
  - 降低替换模式刷新 UI 时的卡顿与异常概率
...
- **修复** 接收区偶发空行
  - 合并跨串口包拆开的 CRLF
  - 避免显示层把拆包的回车换行渲染为额外空行
- **优化** 输出显示稳定性
  - 保持发送, 接收, 系统, 错误来源的分类显示
  - 改进连续输出场景下的文本追加逻辑

### GUI/CLI 打包入口

- **新增** GUI 专用入口
  - `gui_main.py` 作为纯 GUI 启动入口
  - 打包后生成 GUI 专用可执行文件
- **新增** CLI 专用入口
  - `cli_main.py` 作为纯 CLI 启动入口
  - 打包后生成 CLI 专用可执行文件
- **改进** 通用入口
  - `main.py` 保持兼容模式
  - 默认启动 GUI
  - 传入 `--cli` 时进入 CLI 模式
- **优化** 配置文件识别
  - 可执行文件名末尾 `_GUI`, `_gui`, `_CLI`, `_cli` 自动忽略
  - 通用版, GUI 版和 CLI 版默认共用同一份配置
- **更新** PyInstaller 配置
  - `main.spec` 同时打包兼容版, GUI 专用版和 CLI 专用版
  - 统一图标, hiddenimports 和资源路径配置
